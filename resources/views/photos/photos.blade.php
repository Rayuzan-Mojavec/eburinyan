...
@section('content')
    <div class="p-8 m-32 bg-slate-400 bg-opacity-30 rounded-lg shadow-xl">
                ...
                <div class="grid grid-cols-3 gap-4 mx-auto justify-items-center">
                    @foreach ($photos as $photo)
                        <a href="{{ route('photos.show', $photo->id) }}" class="mx-auto self-start">
                            <img class="rounded-md drop-shadow-xl mx-auto max-h-[560px] border"
                                src="{{ asset('storage/' . $photo->photos) }}" alt="" />
                        </a>
                    @endforeach
                </div>
            @else
